...
{$ENDIF !NEXTGEN}
  ...
  begin
    ...
    if ctor = nil then
      ...
      // P4D fix: .AsType<TCustomAttribute> fails cross-module on Linux because
      // each .so has a distinct TypeInfo(TCustomAttribute).  Hard cast is safe
      // here -- the object was just constructed by its own attribute constructor.
      Result := TCustomAttribute(ctor.Invoke(attrType.MetaclassType,
        ParseArgs(attrType, paramData, paramLen, ctor.GetParameters)).AsObject);
  end;
...
begin
  Result := AnsiSameText(AName, Name);
end;
...
class function TRttiTypeHelper.GetObjectListInVisOrder<T>(const AType: TRttiType; const AGetListFunc: TRttiType.TGetListFunc<T>): TArray<T>;
var
  flat: TArray<TArray<T>>;
  tp: TRttiType;
  bases: array [0..31] of TRttiType;
  depth: Integer;
begin
  tp := AType;
  depth := 0;
...
class function TRttiTypeHelper.GetObjectListInDeclOrder<T>(const AType: TRttiType; const AGetListFunc: TRttiType.TGetListFunc<T>): TArray<T>;
var
  flat: TArray<TArray<T>>;
  tp: TRttiType;
  bases: array [0..31] of TRttiType;
...
begin
  Result := TRttiTypeHelper.GetObjectListInVisOrder<TRttiField>(Self, _GetDeclaredFields);
end;

class function TRttiTypeHelper.GetFieldsInDeclOrder(const AType: TRttiType): TArray<TRttiField>;
begin
  Result := GetObjectListInDeclOrder<TRttiField>(AType, _GetDeclaredFields);
end;
...
begin
  Result := TRttiTypeHelper.GetObjectListInVisOrder<TRttiIndexedProperty>(Self, _GetDeclaredIndexedProperties);
end;

class function TRttiTypeHelper.GetIndexedPropertiesInDeclOrder(const AType: TRttiType): TArray<TRttiIndexedProperty>;
begin
  Result := GetObjectListInDeclOrder<TRttiIndexedProperty>(AType, _GetDeclaredIndexedProperties);
end;
...
begin
  Result := TRttiTypeHelper.GetObjectListInVisOrder<TRttiMethod>(Self, _GetDeclaredMethods);
end;

class function TRttiTypeHelper.GetMethodsInDeclOrder(const AType: TRttiType): TArray<TRttiMethod>;
begin
  Result := GetObjectListInDeclOrder<TRttiMethod>(AType, _GetDeclaredMethods);
end;
...
begin
  Result := TRttiTypeHelper.GetObjectListInVisOrder<TRttiProperty>(Self, _GetDeclaredProperties);
end;

class function TRttiTypeHelper.GetPropertiesInDeclOrder(const AType: TRttiType): TArray<TRttiProperty>;
begin
  Result := GetObjectListInDeclOrder<TRttiProperty>(AType, _GetDeclaredProperties);
end;
...
begin
  Result := Pool.TypeOrNil(TypeData^.ParentInfo);
end;
...
begin
  Result := TRttiTypeHelper.GetObjectListInVisOrder<TRttiInterfaceType>(Self, _GetDeclaredImplementedInterfaces);
end;
...
{$IFDEF WIN32}
    ...
    Inc(P, SizeOf(Word)) // size of Attribute field
{$ENDIF Win32}
  ;
end;
...
begin
  Result := Pool.TypeOrNil(TypeData^.IntfParent);
end;
