...
class TestLogger:
    ...
    @classmethod
    def _run_logger_subprocess(cls, output_system, message):
        root = Path(__file__).resolve().parent.parent
        platform_dir = cls._platform_dir()
        script = (
            "import sys\n"
            "from pathlib import Path\n"
    ...
    def test_logger_writes_stdout(self):
        ...
        result = self._run_logger_subprocess("stdout", marker)
    ...
    def test_logger_writes_stderr(self):
        ...
        result = self._run_logger_subprocess("stderr", marker)
        assert result.returncode == 0, result.stderr
